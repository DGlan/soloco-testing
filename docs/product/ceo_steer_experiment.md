...
**结论**（均为 **n=1 单次观测**，见第六节局限）

1. 前置约束显著有效：C 相对 A 成本降 55%、轮数 4→1、自建部门 2→0。
2. 一次性 steer 无效甚至有害：规划期发会打死使命，执行期发被无视。
3. `--autonomy bounded` 替代不了显式约束，**与之叠加反而更差**（F 花 2.2 倍钱干更少的活）。
4. **数量型约束**（"新增不超过 2 个"）可靠；**语义型约束**（"只做最小必要改动"）不可靠。
5. ⚠️ **"约束能保护用户数据"这一论点已被 F 组推翻，不要写进提案**（详见第三节第 5 条）。

**「常驻要求」是刚需，且不构成对 `bounded` 的重复造轮子；但提案应主张"引导用户写可验证的
数量型约束"，而非泛泛的态度性表述。**
...
| 编号 | 内容 | 状态 |
|---|---|---|
| **D** | `--autonomy bounded` 档 | ✅ 已完成 —— bounded 替代不了显式约束 |
| **F** | `bounded` + 显式约束叠加 | ✅ 已完成 —— 叠加更差，工具受限导致返工 |
| **E** | **重复 A/C 各 3 次以上** | ⬜ **最高优先级**。F 已经证明 n=1 会产生假结论（见第 5 条），A/C 的核心差距必须复现验证 |
| **B2-1** | 重跑执行期 steer，全程不中断 | ⬜ 待补。现有 B2 终态数据不可与其他组并列 |
| **G** | 换任务类型（非目录整理）复验 | ⬜ 待补。当前结论只在单一任务类型上成立 |

> **E 组现在是最该做的。** F 推翻了"约束保护用户数据"之后，
> 剩下的核心结论（C 省 55% 成本、轮数 4→1）同样建立在 n=1 上，
> 存在被同样方式推翻的风险。**在补 E 之前，提案里所有数字都应标注"单次观测"。**
